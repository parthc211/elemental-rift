...
public class QuickDoor : MonoBehaviour {
	...
	void CloseSlide()
	{
		if (moveType == moveEnum.smooth)
		{
			...
			transform.position = Vector3.Lerp (startPos, OpenPosition(), openPercent);
		} else if (moveType == moveEnum.linear)
		{
			transform.position = Vector3.Lerp (startPos, OpenPosition(), openPercent = Mathf.Clamp (openPercent -= Time.fixedDeltaTime / duration, 0, 1));
		}
	...
	Vector3 OpenPosition()
	{
		...
		case doorEnum.slideUp:
			...
			openPos = startPos + (Vector3.up * moveDistance);
			break;
		case doorEnum.slideDown:
			if (!customMoveDistance)
				moveDistance = doorHeight;
			
			openPos = startPos + (Vector3.down * moveDistance);
			break;
		case doorEnum.slideLeft:
			if (!customMoveDistance)
				moveDistance = doorWidth;
			
			openPos = startPos + (Vector3.left * moveDistance);
			break;
		case doorEnum.slideRight:
			if (!customMoveDistance)
				moveDistance = doorWidth;
			
			openPos = startPos + (Vector3.right * moveDistance);
			break;
		case doorEnum.slideForward:
			if (!customMoveDistance)
				moveDistance = doorDepth;
			
			openPos = startPos + (Vector3.forward * moveDistance);
			break;
		case doorEnum.slideBackward:
			if (!customMoveDistance)
				moveDistance = doorDepth;
			
			openPos = startPos + (Vector3.back * moveDistance);
			break;
		case doorEnum.pivot:
