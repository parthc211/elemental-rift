...
public class QuickTrigger : MonoBehaviour {
	...
	void Start(){
		...
		{
			Debug.LogError (gameObject.name + " | Quick Trigger has no interactable tags. It will never be activated!");
		}
...
//			}
		...
		if (GetComponent<Collider> ().isTrigger == false)
			Debug.LogError (gameObject.name + " | Quick Trigger's associated Collider is not a Trigger! Please check if 'Is Trigger' is ticked on the Collider component.");
		else if (GetComponent<Collider> () == null)
			Debug.LogError (gameObject.name + " | Quick Trigger has no Collider! It needs a Collider with 'Is Trigger' ticked in order to work.");
	}
