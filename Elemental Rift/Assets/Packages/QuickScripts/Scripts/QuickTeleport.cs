...
public class QuickTeleport : MonoBehaviour {
	...
	void ErrorMessages()
	{
		...
		if (interactableTags.Count == 0)
			Debug.LogError (this.name + " | Quick Teleporter has no interactable tags. Nothing will ever be teleported!");
		
		if (entrySoundFX != null && entryAudioSource == null)
			Debug.LogError (this.name + " | Quick Teleporter has Entry Audio FX but no Entry Audio Source!");

		if (exitSoundFX != null && exitAudioSource == null)
			Debug.LogError (this.name + " | Quick Teleporter has Exit Audio FX but no Exit Audio Source!");
	}
